...
public class JwtService {
    ...
    private static final String ISSUER = "aimon.memory";
    ...
    public JwtService(JwtProperties properties) {
        if (properties.secret() == null || properties.secret().isBlank()) {
            throw new MemoryException("missing_config",
                    "aimon.memory.jwt.secret is not set. Export AIMON_MEMORY_JWT_SECRET with at least "
                            + MINIMUM_SECRET_BYTES
                            + " bytes of random data (openssl rand -base64 48) before starting.");
        }
        byte[] secret = properties.secret().getBytes(StandardCharsets.UTF_8);
        if (secret.length < MINIMUM_SECRET_BYTES) {
            throw new MemoryException("weak_jwt_secret",
                    "aimon.memory.jwt.secret must be at least " + MINIMUM_SECRET_BYTES + " bytes for HS256");
        }
        ...
        //
        // MAX_LIFETIME was enforced at issue time alone, so aimon.memory.jwt.lifetime: 60d booted cleanly and
        ...
        if (properties.lifetime() == null || properties.lifetime().isNegative() || properties.lifetime().isZero()) {
            throw new MemoryException("missing_config", "aimon.memory.jwt.lifetime must be a positive duration");
        }
        if (properties.lifetime().compareTo(MAX_LIFETIME) > 0) {
            throw new MemoryException("bad_config",
                    "aimon.memory.jwt.lifetime is " + properties.lifetime() + ", above the " + MAX_LIFETIME.toDays()
    ...
    public String issue(MemoryPrincipal principal, Duration lifetime) {
        Instant now = Instant.now();
        Duration ttl = lifetime == null ? defaultLifetime : lifetime;
        if (ttl.isNegative() || ttl.isZero()) {
            throw new MemoryException("bad_lifetime", "a token lifetime must be positive");
        }
        if (ttl.compareTo(MAX_LIFETIME) > 0) {
            throw new MemoryException("bad_lifetime", "a token lifetime may not exceed " + MAX_LIFETIME.toDays()
                    + " days; there is no" + " revocation list, so expiry is the only thing that ends a leaked token");
        }
        ...
        return Jwts.builder().issuer(ISSUER).issuedAt(Date.from(now)).expiration(Date.from(now.plus(ttl)))
                .claims(claims).signWith(key).compact();
    }

    public MemoryPrincipal verify(String token) {
        try {
            Claims claims = Jwts.parser().requireIssuer(ISSUER).verifyWith(key).build().parseSignedClaims(token)
                    .getPayload();
            if (claims.getExpiration() == null) {
                throw new UnauthorizedException("token has no expiry");
            }
            return new MemoryPrincipal(TokenScope.fromWire(String.valueOf(claims.get("scope"))), string(claims, "ws"),
                    string(claims, "peer"), string(claims, "sess"), Boolean.TRUE.equals(claims.get("amr")));
