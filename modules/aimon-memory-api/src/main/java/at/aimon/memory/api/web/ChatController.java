...
public class ChatController {
    ...
    public Dtos.ChatResponse chat(@PathVariable String workspace, MemoryPrincipal principal,
        ...
        return new Dtos.ChatResponse(result.text(), result.iterations(), result.stoppedAtLimit(), result.calls()
                .stream().map(c -> new Dtos.ToolCallResponse(c.name(), c.argumentsJson(), c.failed())).toList());
    }
    ...
    private DialecticService.Question question(String workspace, MemoryPrincipal principal, Requests.ChatRequest body) {
        ...
        List<LlmMessage> history = body.history() == null
                ? List.of()
                : body.history().stream()
                        .map(t -> new LlmMessage("assistant".equalsIgnoreCase(t.role()) ? Role.ASSISTANT : Role.USER,
                                t.content()))
                        .toList();
        String schema = null;
        ...
        return new DialecticService.Question(pair, body.session(), body.question(), history,
