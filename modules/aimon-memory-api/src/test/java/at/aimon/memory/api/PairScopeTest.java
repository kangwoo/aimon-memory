...
class PairScopeTest extends ApiTestBase {
    ...
    void seed() throws Exception {
        mvc.perform(post("/v1/workspaces/ws").header("Authorization", bearer(adminToken()))).andExpect(status().isOk());
    ...
    void aPeerTokenCannotRecallAnotherPeersMemory() throws Exception {
        mvc.perform(post("/v1/workspaces/ws/recall").contentType(MediaType.APPLICATION_JSON)
                ...
                .header("Authorization", bearer(peerToken("ws", "bob")))).andExpect(status().isForbidden())
                .andExpect(jsonPath("$.code").value("forbidden"));

        mvc.perform(post("/v1/workspaces/ws/recall").contentType(MediaType.APPLICATION_JSON)
                .content("{\"query\":\"salary\",\"observer\":\"bob\",\"observed\":\"carol\"}")
                .header("Authorization", bearer(peerToken("ws", "bob")))).andExpect(status().isOk());
    }
    ...
    void aPeerTokenMayObserveAnyone() throws Exception {
        mvc.perform(get("/v1/workspaces/ws/conclusions").param("observer", "bob").param("observed", "alice")
                .header("Authorization", bearer(peerToken("ws", "bob")))).andExpect(status().isOk());
    }
    ...
    void everyRouteThatNamesAPairChecksIt() throws Exception {
        ...
        mvc.perform(get("/v1/workspaces/ws/conclusions").param("observer", "alice").param("observed", "alice")
                .header("Authorization", bob)).andExpect(status().isForbidden());

        mvc.perform(get("/v1/workspaces/ws/recall/provenance").param("entity", "seoul").param("observer", "alice")
                .param("observed", "alice").header("Authorization", bob)).andExpect(status().isForbidden());

        mvc.perform(get("/v1/workspaces/ws/peer-card").param("observer", "alice").param("observed", "alice")
                .header("Authorization", bob)).andExpect(status().isForbidden());

        mvc.perform(post("/v1/workspaces/ws/peer-card/refresh").param("observer", "alice").param("observed", "alice")
                .header("Authorization", bob)).andExpect(status().isForbidden());
    ...
    void theReasoningChainWillNotFetchAnotherPairsConclusion() throws Exception {
        ...
        mvc.perform(get("/v1/workspaces/ws/conclusions/" + id + "/chain").param("observer", "bob")
                .param("observed", "bob").header("Authorization", bearer(peerToken("ws", "bob"))))
                .andExpect(status().isNotFound());

        mvc.perform(get("/v1/workspaces/ws/conclusions/" + id + "/chain").param("observer", "alice")
                .param("observed", "alice").header("Authorization", bearer(peerToken("ws", "alice"))))
                .andExpect(status().isOk());
    }
    ...
    void anIdAloneDoesNotReachAnotherPairsConclusion() throws Exception {
        ...
        mvc.perform(get("/v1/workspaces/ws/conclusions/" + id + "/events").header("Authorization",
                bearer(peerToken("ws", "alice")))).andExpect(status().isOk())
                .andExpect(jsonPath("$[0].event").value("add"));
        mvc.perform(
                delete("/v1/workspaces/ws/conclusions/" + id).header("Authorization", bearer(peerToken("ws", "alice"))))
                .andExpect(status().isOk());
    }
    ...
    void theOwnerCanStillReadTheAuditTrailAfterDeleting() throws Exception {
        ...
        mvc.perform(get("/v1/workspaces/ws/conclusions/" + id + "/events").header("Authorization", alice))
                .andExpect(status().isOk()).andExpect(jsonPath("$[0].event").value("delete"));
    }
    ...
    void anUnknownConclusionIdIsNotFound() throws Exception {
        mvc.perform(get("/v1/workspaces/ws/conclusions/nope/events").header("Authorization",
                bearer(peerToken("ws", "alice")))).andExpect(status().isNotFound());
    }

    private String injectAliceConclusion() throws Exception {
        String created = mvc
                .perform(post("/v1/workspaces/ws/conclusions").contentType(MediaType.APPLICATION_JSON)
                        .content("{\"observer\":\"alice\",\"observed\":\"alice\",\"session\":\"s1\","
     ...
     * at.aimon.memory.core.key} where the rule cannot see it, and {@link WorkUnitKey#pair()} hands one back
    ...
    void noControllerBuildsAPairKeyWithoutGoingThroughPairScope() {
        Path classes = repositoryRoot().resolve("modules/aimon-memory-api/build/classes/java/main");
        org.assertj.core.api.Assertions.assertThat(Files.isDirectory(classes))
                .as("compiled API classes found; run ./gradlew classes if this fails").isTrue();

        JavaClasses imported = new ClassFileImporter().importPath(classes);
        noClasses().that().resideInAPackage("at.aimon.memory.api.web..").should()
                .callConstructor(PairKey.class, String.class, String.class, String.class).orShould()
                .callMethod(PairKey.class, "self", String.class, String.class).orShould()
                .callConstructor(WorkUnitKey.class, TaskType.class, String.class, String.class, String.class,
                        String.class)
