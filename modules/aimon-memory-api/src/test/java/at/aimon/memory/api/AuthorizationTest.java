...
class AuthorizationTest extends ApiTestBase {
    ...
    void anUnauthenticatedRequestIsRejected() throws Exception {
        mvc.perform(get("/v1/workspaces/ws")).andExpect(status().isUnauthorized())
    ...
    void aNarrowTokenCannotReachABroadRoute() throws Exception {
        ...
        mvc.perform(get("/v1/workspaces").header("Authorization", bearer(adminToken()))).andExpect(status().isOk());
    }
    ...
    void aWorkspaceTokenCanMintANarrowerTokenForItsOwnWorkspace() throws Exception {
        ...
        mvc.perform(post("/v1/tokens").contentType(MediaType.APPLICATION_JSON).content(body).header("Authorization",
                bearer(workspaceToken("ws")))).andExpect(status().isOk())
                .andExpect(jsonPath("$.scope").value("session"));

        mvc.perform(post("/v1/tokens").contentType(MediaType.APPLICATION_JSON).content(body).header("Authorization",
                bearer(adminToken()))).andExpect(status().isOk()).andExpect(jsonPath("$.scope").value("session"));
    }
    ...
    void mintingCannotWidenTheTokenThatAsked() throws Exception {
        ...
        mvc.perform(post("/v1/tokens").contentType(MediaType.APPLICATION_JSON).content("{\"scope\":\"admin\"}")
                .header("Authorization", bearer(workspaceToken("ws")))).andExpect(status().isForbidden());
    ...
    void aPeerOrSessionTokenCannotMintAnything() throws Exception {
        ...
        mvc.perform(post("/v1/tokens").contentType(MediaType.APPLICATION_JSON).content(body).header("Authorization",
                bearer(peerToken("ws", "alice")))).andExpect(status().isForbidden());

        mvc.perform(post("/v1/tokens").contentType(MediaType.APPLICATION_JSON).content(body).header("Authorization",
                bearer(sessionToken("ws", "s1", true)))).andExpect(status().isForbidden());
    }
    ...
    void aScopedTokenMustNameWhatItIsScopedTo() throws Exception {
        mvc.perform(post("/v1/tokens").contentType(MediaType.APPLICATION_JSON)
                .content("{\"scope\":\"session\",\"workspace\":\"ws\"}").header("Authorization", bearer(adminToken())))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("bad_scope"));
    }
    ...
    void aSessionTokenCannotReachAnotherSession() throws Exception {
        mvc.perform(
                get("/v1/workspaces/ws/sessions/other").header("Authorization", bearer(sessionToken("ws", "s1", true))))
                .andExpect(status().isForbidden());
    }
    ...
    void memberReadNeedsBothTheRouteAndTheTokenToAllowIt() throws Exception {
        ...
        mvc.perform(
                get("/v1/workspaces/ws/sessions/s1").header("Authorization", bearer(sessionToken("ws", "s1", true))))
                .andExpect(status().isOk());

        mvc.perform(
                get("/v1/workspaces/ws/sessions/s1").header("Authorization", bearer(sessionToken("ws", "s1", false))))
                .andExpect(status().isForbidden());
        ...
        mvc.perform(post("/v1/workspaces/ws/recall").contentType(MediaType.APPLICATION_JSON)
                ...
                .header("Authorization", bearer(sessionToken("ws", "s1", true)))).andExpect(status().isForbidden());
    }
    ...
    void aSessionTokenStillSpeaksForEveryoneInItsOwnSession() throws Exception {
        ...
        mvc.perform(post("/v1/workspaces/ws/sessions/s1/messages").contentType(MediaType.APPLICATION_JSON)
                ...
                .header("Authorization", bearer(sessionToken("ws", "s1", false)))).andExpect(status().isOk());

        mvc.perform(post("/v1/workspaces/ws/sessions/other/messages").contentType(MediaType.APPLICATION_JSON)
                .content("{\"messages\":[{\"peer\":\"alice\",\"content\":\"hello\"}]}")
                .header("Authorization", bearer(sessionToken("ws", "s1", false)))).andExpect(status().isForbidden());
    }
    ...
    void aPeerTokenCannotActAsAnotherPeer() throws Exception {
        ...
        mvc.perform(get("/v1/workspaces/ws/peers/alice").header("Authorization", bearer(peerToken("ws", "bob"))))
                .andExpect(status().isForbidden());

        mvc.perform(get("/v1/workspaces/ws/peers/alice").header("Authorization", bearer(peerToken("ws", "alice"))))
                .andExpect(status().isOk());
    }
}
