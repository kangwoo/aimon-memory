...
class ApiActuatorTest {
    ...
    void metricsAreServedOnTheManagementPort() throws Exception {
        ...
        assertThat(response.body()).as("panels in docs/dashboards/aimon-memory-overview.json query these by name")
                .contains("http_server_requests_seconds").contains("jvm_memory_used_bytes");
    }
    ...
    void theServicePortServesNoActuatorEndpoints() throws Exception {
        ...
        assertThat(get(servicePort, "/actuator/prometheus").statusCode())
                .as("metrics must not be reachable on the connector that answers /v1").isEqualTo(404);
