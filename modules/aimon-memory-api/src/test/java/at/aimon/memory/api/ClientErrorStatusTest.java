...
class ClientErrorStatusTest extends ApiTestBase {
    ...
    void aTruncatedBodyIsABadRequest() throws Exception {
        mvc.perform(post("/v1/tokens").header("Authorization", bearer(token)).contentType(MediaType.APPLICATION_JSON)
    ...
    void anUnsupportedContentTypeIsABadRequest() throws Exception {
        mvc.perform(post("/v1/tokens").header("Authorization", bearer(token)).contentType(MediaType.TEXT_PLAIN)
                .content("scope=admin")).andExpect(status().isBadRequest());
    }
    ...
    void contextForAnUnknownSessionIsNotFound() throws Exception {
        mvc.perform(get("/v1/workspaces/ws/sessions/does-not-exist/context").header("Authorization", bearer(token)))
    ...
    void anUnknownVersionedPathIsNotFound() throws Exception {
        mvc.perform(get("/v1/nope").header("Authorization", bearer(token))).andExpect(status().isNotFound())
