...
class ConfigurationValidationTest extends ApiTestBase {
    ...
    private org.springframework.test.web.servlet.ResultActions configure(String configuration) throws Exception {
        return mvc.perform(put("/v1/workspaces/ws/configuration").header("Authorization", bearer(token))
                .contentType(MediaType.APPLICATION_JSON).content("{\"configuration\":" + configuration + "}"));
    }
    ...
    void aValidConfigurationIsStored() throws Exception {
        configure("{\"language\":\"ko\",\"recall.half_life_days\":30,\"recall.weights\":[0.4,0.3,0.1,0.1,0.05,0.05]}")
                .andExpect(status().isOk()).andExpect(jsonPath("$.configuration.language").value("ko"));
    }
    ...
    void aPeerMayStillCarryItsOwnOpaqueConfiguration() throws Exception {
        mvc.perform(post("/v1/workspaces/ws/peers/bob").header("Authorization", bearer(token))
                .contentType(MediaType.APPLICATION_JSON).content("{\"configuration\":{\"client.avatar\":\"cat.png\"}}"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.configuration['client.avatar']").value("cat.png"));
    }
}
