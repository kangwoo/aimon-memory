...
public class QueueMetrics implements AutoCloseable {
    ...
    public QueueMetrics(QueueRepository queue, MeterRegistry meters) {
        this.queue = queue;
        Gauge.builder("aimon.memory.queue.pending", pending, AtomicLong::doubleValue)
                .description("Unprocessed queue items across all workspaces").register(meters);
        Gauge.builder("aimon.memory.queue.oldest.seconds", oldestSeconds, AtomicLong::doubleValue)
                .description("Age of the oldest unprocessed queue item").baseUnit("seconds").register(meters);
    }
    ...
    public void start() {
        ...
        running = true;
        sampler = Thread.ofPlatform().name("aimon-memory-queue-metrics").daemon().start(this::sampleForever);
    }
