 ...
 *
 * @param weights how much each of the six signals contributes; they must sum to 1.00
 * @param halfLifeDays recency decay; 180 is a deliberately conservative default, not a tuned one
 * @param threshold cut applied to the fused score, not to the semantic score alone
 ...
 */
public record RecallSettings(FusionWeights weights, double halfLifeDays, double threshold, int oversample,
        int entityTopK, double entitySimCut) {

    public static final RecallSettings DEFAULT = new RecallSettings(FusionWeights.DEFAULT, 180.0, 0.0, 4, 10, 0.5);
