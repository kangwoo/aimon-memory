...
public sealed interface Filter {
    ...
    static Filter parse(Map<String, ?> raw) {
        ...
        if (nodes > MAX_NODES) {
            throw new FilterException("filter has " + nodes + " predicates; at most " + MAX_NODES + " are accepted");
        }
        return parsed;
    }
    ...
    private static Filter parse(Map<String, ?> raw, int depth) {
        if (depth > MAX_DEPTH) {
            throw new FilterException("filter is nested more than " + MAX_DEPTH + " levels deep");
        }
