...
public final class Precision {
    ...
    public static void assertMatches(String label, double actual, JsonNode expected) {
        assertThat(expected.isNumber()).as("%s: fixture value is missing or not a number", label).isTrue();
        assertThat(round(actual)).as("%s", label).isEqualTo(round(expected.asDouble()));
    }
}
