...
public final class PostgresSupport {
    ...
    public static synchronized DataSource dataSource() {
        if (dataSource == null) {
            container = new PostgreSQLContainer<>(IMAGE).withDatabaseName("aimon.memory").withUsername("aimon.memory")
                    .withPassword("aimon.memory");
            container.start();
            ...
            config.setPoolName("aimon-memory-test");
    ...
    private static void migrate(DataSource source) {
        Flyway.configure().dataSource(source).locations("classpath:db/migration")
                ...
                .placeholders(java.util.Map.of("embedding_dimensions", "1536")).load().migrate();
    }
    ...
    public static void truncateAll() {
        try (var connection = dataSource().getConnection(); var statement = connection.createStatement()) {
            statement.execute("TRUNCATE workspaces, peers, sessions, session_peers, messages, collections,"
