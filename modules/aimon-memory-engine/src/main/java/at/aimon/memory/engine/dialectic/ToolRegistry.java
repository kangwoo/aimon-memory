...
public class ToolRegistry {
    ...
    private ToolDef searchMessagesTool(PairKey pair, String sessionName) {
        return new ToolDef(SEARCH_MESSAGES,
    ...
    private ToolDef grepMessagesTool(PairKey pair, String sessionName) {
        return new ToolDef(GREP_MESSAGES,
    ...
    // The eclipse formatter (Spotless, this repository's formatting authority) closes a lambda passed
    // after a text block at this column; Checkstyle's Indentation check wants a different one. The
    // checker yields at the two sites where they disagree.
    @SuppressWarnings("checkstyle:Indentation")
    private ToolDef messagesByDateTool(PairKey pair, String sessionName) {
        return new ToolDef(MESSAGES_BY_DATE,
    ...
    private ToolDef searchTemporalTool(PairKey pair) {
        return new ToolDef(SEARCH_TEMPORAL,
    ...
    private ToolDef reasoningChainTool(PairKey pair) {
        return new ToolDef(REASONING_CHAIN,
                ...
                schema("conclusion_id", "The id of a conclusion returned by another tool."), arguments -> {
                    ...
                    return conclusions.find(pair.workspaceName(), id).map(conclusion -> {
                        ...
                        trace.premises().forEach(
                                p -> sb.append("  [").append(p.id()).append("] ").append(p.content()).append('\n'));
                        sb.append("supports:\n");
                        List<Conclusion> dependents = provenance.dependents(pair.workspaceName(), id);
                        if (dependents.isEmpty()) {
                            sb.append("  (nothing)\n");
                        }
                        dependents.forEach(
                                d -> sb.append("  [").append(d.id()).append("] ").append(d.content()).append('\n'));
    ...
    private ToolDef entityProvenanceTool(PairKey pair) {
        return new ToolDef(ENTITY_PROVENANCE,
                ...
                schema("entity", "The name, as it appears in the conversation."), arguments -> {
                    ...
                    return provenance.forEntity(pair, name, DEFAULT_LIMIT).map(result -> {
                        ...
                        result.conclusions().forEach(c -> {
                            sb.append("- [").append(c.conclusion().id()).append("] ").append(c.conclusion().content())
                                    .append('\n');
                            c.sourceMessages().forEach(m -> sb.append("    said: ").append(m.peerName()).append(": ")
    ...
    private static String renderHits(List<ScoredConclusion> hits) {
        ...
        for (ScoredConclusion hit : hits) {
            ...
            sb.append('[').append(conclusion.id()).append("] ").append(conclusion.content()).append("  (confirmed ")
                    .append(conclusion.timesDerived())
                    .append(conclusion.timesDerived() == 1 ? " time, last " : " times, last ")
                    .append(conclusion.lastReinforcedAt()).append(", ").append(conclusion.level().wire()).append(")\n");
        }
        return sb.toString();
    }
    ...
    private static String renderMessages(List<Message> found) {
        ...
        for (Message message : found) {
            sb.append(message.createdAt()).append(' ').append(message.peerName()).append(": ").append(message.content())
                    .append('\n');
        }
        return sb.toString();
    }
