...
public class DialecticService {
    ...
    private static final String FINDINGS_INSTRUCTION = "Answer from these results alone."
            + " The search was cut short at its iteration limit, so say so if what is here does not"
            + " settle the question.";

    /** What the header and the closing instruction cost, reserved out of the findings budget. */
    private static final int FINDINGS_FRAMING_TOKENS = TokenCounter.count(FINDINGS_HEADER)
            + TokenCounter.count(FINDINGS_INSTRUCTION);
     ...
     */
    public record Question(PairKey pair, String sessionName, String text, List<LlmMessage> history,
            ReasoningLevel level, String responseFormatSchema) {
    ...
    public ToolLoopResult answer(Question question) {
        ...
        List<ToolDef> toolset = tools.toolsFor(question.pair(), question.sessionName(), question.level().toolNames());
    ...
    private LlmRequest requestFor(Question question, List<ToolCall> findings) {
        ...
        ResponseFormat format = question.responseFormatSchema() == null
                ? null
                : ResponseFormat.strict("dialectic_answer",
                        ResponseSchemaGuard.validate(question.responseFormatSchema()));

        return new LlmRequest(null, Prompts.dialectic(question.pair().observer(), question.pair().observed()), messages,
                0.0, null, format);
    }
    ...
    static String renderFindings(List<ToolCall> findings) {
        ...
        // nothing else.
        return FINDINGS_HEADER + TokenCounter.truncate(sb.toString(), MAX_FINDINGS_TOKENS - FINDINGS_FRAMING_TOKENS)
