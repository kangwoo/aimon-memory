...
import at.aimon.memory.engine.summarize.Summary;
...
class ContextBudgetTest extends MemoryTestBase {
    ...
    @BeforeEach
    void wire() {
        summarizer = new SummarizerService(stub("{\"summary\":\"alice and bob discussed the project timeline.\"}"),
    ...
    void aLongSessionStillShowsItsMostRecentMessages() {
        ...
        assertThat(result.messages().get(result.messages().size() - 1).content())
                .as("the newest message in the session").isEqualTo("message 2499");
        assertThat(result.messages()).allSatisfy(m -> assertThat(m.seqInSession()).isGreaterThan(2480L));
    }
