...
class ObserverResolverTest {
    ...
    void bothSwitchesOnProducesSelfAndObserverPairs() {
        List<PairKey> pairs = ObserverResolver.observersOf(message("alice"),
                List.of(member("alice", true, true), member("bob", true, true)), WorkspaceSettings.DEFAULT);

        assertThat(pairs).containsExactly(PairKey.self("ws", "alice"), new PairKey("ws", "bob", "alice"));
    }

    @Test
    void observeMeOffDropsTheSelfPair() {
        List<PairKey> pairs = ObserverResolver.observersOf(message("alice"),
                List.of(member("alice", false, true), member("bob", true, true)), WorkspaceSettings.DEFAULT);
    ...
    void observeOthersOffDropsThatListenerOnly() {
        ...
        assertThat(pairs).containsExactly(PairKey.self("ws", "alice"), new PairKey("ws", "carol", "alice"));
    }

    /** Narrowest scope wins: a session-level null defers to the workspace default. */
    @Test
    void nullOverridesFallBackToTheWorkspaceDefault() {
        WorkspaceSettings quiet = new WorkspaceSettings("und", at.aimon.memory.core.config.RecallSettings.DEFAULT,
                at.aimon.memory.core.config.DedupSettings.DEFAULT, at.aimon.memory.core.config.BatchSettings.DEFAULT,
                false, false);

        assertThat(ObserverResolver.observersOf(message("alice"),
                List.of(member("alice", null, null), member("bob", null, null)), quiet)).isEmpty();

        // An explicit session-level true overrides a workspace default of false.
        assertThat(ObserverResolver.observersOf(message("alice"),
                List.of(member("alice", true, null), member("bob", null, null)), quiet))
    ...
    void peersOutsideTheMembershipWindowAreExcluded() {
        SessionPeer late = new SessionPeer("ws", "s1", "late", true, true, SENT.plusSeconds(60), null);
        SessionPeer departed = new SessionPeer("ws", "s1", "gone", true, true, SENT.minusSeconds(3600),
                SENT.minusSeconds(60));
    ...
    void aSpeakerAbsentFromTheRosterStillObservesThemselves() {
        assertThat(ObserverResolver.observersOf(message("stranger"), List.of(member("bob", true, false)),
                WorkspaceSettings.DEFAULT)).containsExactly(PairKey.self("ws", "stranger"));
    }
}
