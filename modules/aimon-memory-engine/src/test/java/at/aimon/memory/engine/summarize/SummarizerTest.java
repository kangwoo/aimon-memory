...
class SummarizerTest extends MemoryTestBase {
    ...
    void nothingHappensBelowTheThreshold() {
        seedMessages(19);
        assertThat(summarizer(StubLlmClient.returning("{\"summary\":\"x\"}")).refresh(WORKSPACE, "s1")).isEmpty();
    }

    @Test
    void theShortSummaryArrivesFirst() {
        seedMessages(20);
        var produced = summarizer(StubLlmClient.returning("{\"summary\":\"short one\"}")).refresh(WORKSPACE, "s1");
    ...
    void bothSummariesAppearOnceTheLongThresholdIsCrossed() {
        seedMessages(60);
        var produced = summarizer(StubLlmClient.returning("{\"summary\":\"a summary\"}")).refresh(WORKSPACE, "s1");
    ...
    void regenerationIsGivenThePreviousSummaryAndReplacesIt() {
        ...
        assertThat(summarizer(second).find(session, Summary.SHORT).orElseThrow().text()).isEqualTo("second pass");
    }
