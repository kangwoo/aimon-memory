...
class DreamerTest extends MemoryTestBase {
    ...
    private String store(PairKey pair, String content) {
        ...
        return conclusions.upsert(ConclusionDraft.builder().pair(pair).sessionName("s1").content(content)
                ...
                .actor(Actor.DERIVER).build()).conclusionId();
    }
    ...
    void derivedConclusionsRecordTheirPremises() {
        ...
        List<Conclusion> derived = conclusions.list(pair, Filter.ALL, 0, 50).items().stream()
                .filter(c -> c.level() != ConclusionLevel.EXPLICIT).toList();
    ...
    void inductiveConclusionsCarryConfidenceAndATtl() {
        ...
        var inductive = conclusions.list(pair, Filter.ALL, 0, 50).items().stream()
                .filter(c -> c.level() == ConclusionLevel.INDUCTIVE).findFirst().orElseThrow();
        ...
        var deductive = conclusions.list(pair, Filter.ALL, 0, 50).items().stream()
                .filter(c -> c.level() == ConclusionLevel.DEDUCTIVE).findFirst().orElseThrow();
