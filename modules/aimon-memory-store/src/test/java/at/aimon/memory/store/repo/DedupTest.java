...
class DedupTest extends StoreTestBase {
     ...
     */
    private static final List<String> WORDS = List.of("alice", "works", "at", "the", "central", "bank", "in", "seoul",
            "gangnam", "district", "near", "han", "river", "every", "weekday", "morning", "commuting", "by", "subway",
            "line", "two", "from", "her", "apartment", "which", "she", "bought", "last", "year", "together");
    ...
    void stageOneMatchesOnHashAndReinforces() {
        ...
        assertThat(events.history(WORKSPACE, first.conclusionId(), 10)).extracting(e -> e.event())
     ...
     * {@link at.aimon.memory.text.Normalizer} would leave behind.
     */
    @Test
    void stageTwoAbsorbsRowsHashedUnderAnOlderRule() {
        var legacy = at.aimon.memory.core.model.ConclusionDraft.builder().pair(pair()).sessionName("s1")
                .content("alice works at a bank").contentNorm("alice works at a bank")
                .contentAnalyzed(Drafts.analyze("alice works at a bank")).contentHash("0".repeat(64))
                .level(ConclusionLevel.EXPLICIT).embedding(Drafts.embed("alice works at a bank"))
                .actor(at.aimon.memory.core.model.Actor.DERIVER).build();
    ...
    void stageThreeReplacesWhenTheNewPhrasingCarriesMore() {
        ...
        DedupOutcome replacement = conclusions.upsert(Drafts.explicit(pair(), "s1", sentence(30) + " punctually"));
    ...
    void dedupNeverCrossesTheLevel() {
        ...
        DedupOutcome deduced = conclusions
                .upsert(Drafts.of(pair(), null, "alice works at a bank", ConclusionLevel.DEDUCTIVE, List.of()));
    ...
    void concurrentIdenticalWritesConvergeOnOneRow() throws Exception {
        ...
        var stored = conclusions.list(target, at.aimon.memory.core.filter.Filter.ALL, 0, 50).items();
    ...
    void everyWritePathLeavesAnEvent() {
        ...
        conclusions.upsert(Drafts.explicit(pair(), "s1", "alice works at a bank"));
        conclusions.softDelete(WORKSPACE, inserted.conclusionId(), at.aimon.memory.core.model.Actor.API,
                java.util.Map.of(), EventType.DELETE);
        conclusions.restore(WORKSPACE, inserted.conclusionId(), at.aimon.memory.core.model.Actor.API);

        assertThat(events.history(WORKSPACE, inserted.conclusionId(), 10)).extracting(e -> e.event())
