...
class MessageGrepTest extends StoreTestBase {
    ...
    void seedMessages() {
        ...
        messages.insertBatch(WORKSPACE, "s1", start,
                List.of(new MessageRepository.NewMessage("alice", "I work at a Bank in Seoul", 6, Map.of()),
    ...
    void findsAPhraseRegardlessOfCase() {
        assertThat(messages.grep(WORKSPACE, "alice", "s1", "bank in seoul", 10)).singleElement()
    ...
    void wildcardsInTheNeedleAreLiteral() {
        ...
        assertThat(messages.grep(WORKSPACE, "alice", "s1", "%", 10))
                .as("a bare percent must match only the message containing one").hasSize(1);
    ...
    void escapingIsAppliedToEveryWildcardCharacter() {
        assertThat(MessageRepository.escapeForLike("100%_x\\y")).isEqualTo("100\\%\\_x\\\\y");
