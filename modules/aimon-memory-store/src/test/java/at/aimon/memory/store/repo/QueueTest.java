...
class QueueTest extends StoreTestBase {
    ...
    private static final BatchSettings NEVER_READY = new BatchSettings(1_000_000, Duration.ofDays(365),
            Duration.ofDays(365));
    ...
    void onlyOneWorkerClaimsAKey() throws Exception {
        ...
        try (ExecutorService pool = Executors.newVirtualThreadPerTaskExecutor()) {
            List<Callable<Boolean>> attempts = IntStream.range(0, 20)
                    .<Callable<Boolean>>mapToObj(i -> () -> queue.claim(encoded, "worker-" + i, Duration.ofMinutes(1)))
                    .toList();
            long winners = pool.invokeAll(attempts).stream().filter(future -> {
                try {
                    return future.get();
                } catch (Exception e) {
    ...
    void tokenThresholdMakesAUnitReady() {
        ...
        assertThat(queue.ready(BatchSettings.DEFAULT, 10)).singleElement().satisfies(unit -> {
    ...
    void idleFlushMakesASmallBatchReady() {
        ...
        BatchSettings idleNow = new BatchSettings(1_000_000, Duration.ofDays(365), Duration.ofSeconds(-1));
    ...
    void ageMakesALargeQuietBatchReady() {
        queue.enqueue(key(), Map.of("message_id", 1), 5);
        BatchSettings agedOut = new BatchSettings(1_000_000, Duration.ofSeconds(-1), Duration.ofDays(365));
    ...
    void perWorkspaceGatingSeesTheSameUnitsAsAGlobalScan() {
        queue.enqueue(key(), Map.of("message_id", 1), 1000);
        queue.enqueue(new WorkUnitKey(at.aimon.memory.core.key.TaskType.REPRESENTATION, WORKSPACE, "s2", "bob", "bob"),
