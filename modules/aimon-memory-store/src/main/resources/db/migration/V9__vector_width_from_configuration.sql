-- The vector columns follow aimon.memory.embed.dimensions — as a migration of their own, not an edit to V1.
--
...
-- starting at all, with a checksum mismatch on the next boot of aimon-memory-api or aimon-memory-worker. Fresh
...
BEGIN
    ...
    LOOP
        ...
        IF populated THEN
            RAISE EXCEPTION
                '%.embedding is vector(%) and aimon.memory.embed.dimensions is %. Widening it discards every '
                ...
                'aimon.memory.embed.dimensions back to %.',
