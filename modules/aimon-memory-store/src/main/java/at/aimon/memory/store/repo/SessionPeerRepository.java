...
public class SessionPeerRepository {
    ...
    public void join(String workspace, String session, String peer) {
        jdbc.sql("""
                ...
                SET joined_at = CASE WHEN session_peers.left_at IS NULL
                    ...
                    left_at = NULL
                """).params(workspace, session, peer).update();
        ...
        jdbc.sql("""
                ...
                SET observe_me = EXCLUDED.observe_me,
                    ...
                    joined_at = CASE WHEN session_peers.left_at IS NULL
                                     THEN session_peers.joined_at ELSE now() END,
                    left_at = NULL
                """).params(workspace, session, peer, observeMe, observeOthers).update();
        openWindow(workspace, session, peer);
    }
    ...
    public void leave(String workspace, String session, String peer) {
        jdbc.sql("UPDATE session_peers SET left_at = now()"
                + " WHERE workspace_name = ? AND session_name = ? AND peer_name = ? AND left_at IS NULL")
                .params(workspace, session, peer).update();
        jdbc.sql("UPDATE session_peer_windows SET left_at = now()"
                + " WHERE workspace_name = ? AND session_name = ? AND peer_name = ? AND left_at IS NULL")
                .params(workspace, session, peer).update();
    }

    public List<SessionPeer> members(String workspace, String session) {
        return jdbc.sql("SELECT * FROM session_peers WHERE workspace_name = ? AND session_name = ? ORDER BY peer_name")
                .params(workspace, session).query(RowMappers.SESSION_PEER).list();
    }
    ...
    public List<SessionPeer> membersAt(String workspace, String session, Instant when) {
        return jdbc.sql("""
                ...
                .query(RowMappers.SESSION_PEER).list();
    }

    public Optional<SessionPeer> find(String workspace, String session, String peer) {
        return jdbc.sql("SELECT * FROM session_peers WHERE workspace_name = ? AND session_name = ? AND peer_name = ?")
                .params(workspace, session, peer).query(RowMappers.SESSION_PEER).optional();
    }
    ...
    public void replace(String workspace, String session, List<Membership> roster) {
        ...
        jdbc.sql("UPDATE session_peers SET left_at = now()"
                + " WHERE workspace_name = ? AND session_name = ? AND left_at IS NULL" + " AND peer_name <> ALL (?)")
                .params(workspace, session, staying).update();
