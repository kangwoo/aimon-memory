...
public class DreamRepository {
    ...
    public record Dream(String id, String workspaceName, String observer, String observed, String dreamType,
            String status, int conclusionsAtStart, int produced, String error, int requeueCount, Instant createdAt,
            Instant startedAt, Instant completedAt) {
    }

    private static final org.springframework.jdbc.core.RowMapper<Dream> MAPPER = (rs, i) -> new Dream(
            rs.getString("id"), rs.getString("workspace_name"), rs.getString("observer"), rs.getString("observed"),
            rs.getString("dream_type"), rs.getString("status"), rs.getInt("conclusions_at_start"),
            rs.getInt("produced"), rs.getString("error"), rs.getInt("requeue_count"),
    ...
    public Optional<Dream> schedule(PairKey pair, DreamType type, int conclusionsAtStart) {
        ...
        Optional<String> inserted = jdbc.sql("""
                ...
                ON CONFLICT (workspace_name, observer, observed)
                  ...
                  DO NOTHING
                RETURNING id
                """).params(id, pair.workspaceName(), pair.observer(), pair.observed(), type.wire(), conclusionsAtStart)
    ...
    public void complete(String id, int produced) {
        jdbc.sql("UPDATE dreams SET status = 'completed', produced = ?, completed_at = now()" + " WHERE id = ?")
                .params(produced, id).update();
    }

    public void fail(String id, String error) {
        jdbc.sql("UPDATE dreams SET status = 'failed', error = ?, completed_at = now() WHERE id = ?").params(error, id)
                .update();
    }
    ...
    public List<Dream> pending(int limit) {
        return jdbc.sql("SELECT * FROM dreams WHERE status = 'pending' ORDER BY created_at LIMIT ?").param(limit)
                .query(MAPPER).list();
    }

    public List<Dream> forPair(PairKey pair, int limit) {
        return jdbc
                .sql("SELECT * FROM dreams WHERE workspace_name = ? AND observer = ? AND observed = ?"
                        + " ORDER BY created_at DESC LIMIT ?")
                .params(pair.workspaceName(), pair.observer(), pair.observed(), limit).query(MAPPER).list();
    }
}
