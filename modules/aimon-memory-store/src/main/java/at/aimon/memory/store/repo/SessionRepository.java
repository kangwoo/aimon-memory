...
public class SessionRepository {
    ...
    public Session getOrCreate(String workspace, String name, Map<String, Object> metadata,
    ...
    public Optional<Session> find(String workspace, String name) {
        return jdbc.sql("SELECT * FROM sessions WHERE workspace_name = ? AND name = ?").params(workspace, name)
                .query(RowMappers.SESSION).optional();
    }
    ...
    public long nextSequence(String workspace, String session, int count) {
        Long end = jdbc.sql("""
                ...
                """).params(count, workspace, session).query(Long.class).single();
        return end - count + 1;
    }
    ...
    public void updateInternalMetadata(String workspace, String name, Map<String, Object> internalMetadata) {
        jdbc.sql("UPDATE sessions SET internal_metadata = ? WHERE workspace_name = ? AND name = ?")
                .params(Jsonb.of(internalMetadata), workspace, name).update();
    }
    ...
    public void setActive(String workspace, String name, boolean active) {
        jdbc.sql("UPDATE sessions SET is_active = ? WHERE workspace_name = ? AND name = ?")
                .params(active, workspace, name).update();
    }

    public Page<Session> list(String workspace, int page, int size) {
        long total = jdbc.sql("SELECT count(*) FROM sessions WHERE workspace_name = ?").param(workspace)
                .query(Long.class).single();
        var items = jdbc
                .sql("SELECT * FROM sessions WHERE workspace_name = ?"
                        + " ORDER BY created_at DESC, name LIMIT ? OFFSET ?")
                .params(workspace, size, (long) page * size).query(RowMappers.SESSION).list();
