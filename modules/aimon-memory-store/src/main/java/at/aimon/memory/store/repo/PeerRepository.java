...
public class PeerRepository {
    ...
    public Peer getOrCreate(String workspace, String name, Map<String, Object> metadata,
    ...
    public Optional<Peer> find(String workspace, String name) {
        return jdbc.sql("SELECT * FROM peers WHERE workspace_name = ? AND name = ?").params(workspace, name)
                .query(RowMappers.PEER).optional();
    }
    ...
    public void updateConfiguration(String workspace, String name, Map<String, Object> configuration) {
        ...
        jdbc.sql("UPDATE peers SET configuration = ? WHERE workspace_name = ? AND name = ?")
                .params(Jsonb.of(configuration), workspace, name).update();
    }

    public Page<Peer> list(String workspace, int page, int size) {
        long total = jdbc.sql("SELECT count(*) FROM peers WHERE workspace_name = ?").param(workspace).query(Long.class)
                .single();
        var items = jdbc.sql("SELECT * FROM peers WHERE workspace_name = ? ORDER BY name LIMIT ? OFFSET ?")
                .params(workspace, size, (long) page * size).query(RowMappers.PEER).list();
