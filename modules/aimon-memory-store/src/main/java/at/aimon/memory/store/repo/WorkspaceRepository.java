...
public class WorkspaceRepository {
    ...
    public Workspace getOrCreate(String name, Map<String, Object> metadata, Map<String, Object> configuration) {
        ...
        jdbc.sql("""
                ...
                """).params(name, Jsonb.of(metadata), Jsonb.of(configuration)).update();
        return find(name).orElseThrow();
    }

    public Optional<Workspace> find(String name) {
        return jdbc.sql("SELECT * FROM workspaces WHERE name = ?").param(name).query(RowMappers.WORKSPACE).optional();
    }
     ...
     * why — the failure {@link at.aimon.memory.core.config.ConfigurationException} exists to prevent.
     */
    public void updateConfiguration(String name, Map<String, Object> configuration) {
        WorkspaceSettingsService.validate(configuration);
        jdbc.sql("UPDATE workspaces SET configuration = ? WHERE name = ?").params(Jsonb.of(configuration), name)
                .update();
    }

    public Page<Workspace> list(int page, int size) {
        long total = jdbc.sql("SELECT count(*) FROM workspaces").query(Long.class).single();
        var items = jdbc.sql("SELECT * FROM workspaces ORDER BY created_at DESC, name LIMIT ? OFFSET ?")
                .params(size, (long) page * size).query(RowMappers.WORKSPACE).list();
