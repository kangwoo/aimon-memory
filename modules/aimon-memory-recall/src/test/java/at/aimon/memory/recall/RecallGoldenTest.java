...
class RecallGoldenTest extends RecallTestBase {
    ...
    void seedCorpus() {
        ...
        String pattern = store(pair, null, "alice prefers routine over spontaneity", ConclusionLevel.INDUCTIVE,
                List.of());
    ...
    void explainMatchesTheGoldenFixtureToSixDecimals() {
        ...
        for (int i = 0; i < expectedHits.size(); i++) {
            ...
            assertThat(got.path("matched_entities").toString()).as("%s: matched entities", at)
    ...
    void scoreIsTheWeightedSumOfItsSignals() {
        ...
        for (ScoredConclusion hit : response.hits()) {
            assertThat(Precision.round(hit.score())).as("score of %s", hit.conclusion().content())
