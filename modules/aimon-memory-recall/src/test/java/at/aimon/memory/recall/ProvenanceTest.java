...
class ProvenanceTest extends RecallTestBase {
    ...
    private long seedMessage(String session, String peer, String content) {
        ...
        return messages.insertBatch(WORKSPACE, session, start,
                List.of(new MessageRepository.NewMessage(peer, content, 5, Map.of()))).get(0).id();
    }

    private String storeWithSources(PairKey pair, String content, List<String> sourceIds, List<Long> messageIds,
            ConclusionLevel level) {
        String norm = Normalizer.normalize(content);
        return conclusions
                .upsert(ConclusionDraft.builder().pair(pair)
                        .sessionName(level == ConclusionLevel.EXPLICIT ? "s1" : null).content(content).contentNorm(norm)
                        .contentAnalyzed(analyzer.analyze(content)).contentHash(ContentHash.of(norm)).level(level)
                        .sourceIds(sourceIds).messageIds(messageIds)
                        .embedding(embedder.embed(content, EmbedPurpose.DOCUMENT)).actor(Actor.DREAMER).build())
                .conclusionId();
    }
    ...
    void entityWalksThroughToTheOriginalMessages() {
        ...
        String premise = storeWithSources(pair, "alice works at a bank in seoul", List.of(), List.of(messageId),
        ...
        assertThat(result.conclusions()).singleElement().satisfies(entry -> {
            ...
            assertThat(entry.sourceMessages()).singleElement()
                    .satisfies(message -> assertThat(message.content()).contains("Gangnam"));
        });
    }
    ...
    void deductionsInheritTheirEvidenceFromTheirPremises() {
        ...
        String premiseA = storeWithSources(pair, "alice lives in busan", List.of(), List.of(lives),
                ConclusionLevel.EXPLICIT);
        String premiseB = storeWithSources(pair, "alice works in busan", List.of(), List.of(works),
                ConclusionLevel.EXPLICIT);
        String deduction = storeWithSources(pair, "alice does not commute between cities", List.of(premiseA, premiseB),
                List.of(), ConclusionLevel.DEDUCTIVE);
    ...
    void aDeletedPremiseLeavesTheChainAndIsReportedAsMissing() {
        ...
        String premiseA = storeWithSources(pair, "alice lives in busan", List.of(), List.of(said),
                ConclusionLevel.EXPLICIT);
        String premiseB = storeWithSources(pair, "alice works in busan", List.of(), List.of(),
                ConclusionLevel.EXPLICIT);
        String deduction = storeWithSources(pair, "alice does not commute", List.of(premiseA, premiseB), List.of(),
                ConclusionLevel.DEDUCTIVE);

        conclusions.softDelete(WORKSPACE, premiseB, at.aimon.memory.core.model.Actor.API, Map.of(),
                at.aimon.memory.core.model.EventType.DELETE);
    ...
    void aDeletedConclusionIsNotReturnedAsEntityProvenance() {
        PairKey pair = seedPair("alice", "alice");
        seedSession("s1");
        String conclusionId = storeWithSources(pair, "alice lives in seoul", List.of(), List.of(),
                ConclusionLevel.EXPLICIT);
        linkEntity(pair, conclusionId, "seoul");
        assertThat(provenance.forEntity(pair, "seoul", 10).orElseThrow().conclusions()).hasSize(1);

        conclusions.softDelete(WORKSPACE, conclusionId, at.aimon.memory.core.model.Actor.API, Map.of(),
                at.aimon.memory.core.model.EventType.DELETE);
    ...
    void theTreeIsWalkableInBothDirections() {
        ...
        String derived = storeWithSources(pair, "alice is based in the south", List.of(premise), List.of(),
                ConclusionLevel.DEDUCTIVE);
    ...
    void aCycleInTheTreeTerminates() {
        ...
        jdbc.sql("UPDATE conclusions SET source_ids = ?::jsonb WHERE id = ?").params("[\"" + b + "\"]", a).update();
    ...
    void provenanceIsPairScoped() {
        ...
        String conclusionId = storeWithSources(mine, "alice lives in seoul", List.of(), List.of(),
                ConclusionLevel.EXPLICIT);
