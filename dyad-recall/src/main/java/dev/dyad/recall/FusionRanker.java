...
import dev.dyad.recall.signal.EntityBoost;
import dev.dyad.recall.signal.KeywordSignal;
import dev.dyad.recall.signal.RecencySignal;
import dev.dyad.recall.signal.ReinforcementSignal;
...
public final class FusionRanker {
     ...
     */
    public List<ScoredConclusion> rank(
            List<Conclusion> candidates,
            Map<String, Double> semantic,
            Map<String, Double> keywordRaw,
            EntityBoost.Result entities,
            int queryTermCount,
            Instant now,
            int limit) {
        ...
        scored.sort(
                Comparator.comparingDouble(ScoredConclusion::score)
                        .reversed()
                        .thenComparing(ScoredConclusion::id));
    ...
    public Explain explain(
        ...
        return new Explain(
                sem, kw, ent, reinf, rec, lvl, settings.weights(), entities.namesFor(candidate.id()));
    }
