...
public abstract class RecallTestBase {
    ...
    protected String store(
            PairKey pair, String session, String content, ConclusionLevel level, List<String> entityNames) {
        String norm = Normalizer.normalize(content);
        ConclusionDraft draft =
                ConclusionDraft.builder()
                        .pair(pair)
                        .sessionName(level == ConclusionLevel.EXPLICIT ? session : null)
                        .content(content)
                        .contentNorm(norm)
    ...
    protected void setRankingState(String conclusionId, int timesDerived, Instant lastReinforcedAt) {
        jdbc.sql("UPDATE conclusions SET times_derived = ?, last_reinforced_at = ? WHERE id = ?")
                .params(timesDerived, java.sql.Timestamp.from(lastReinforcedAt), conclusionId)
                .update();
    }
    ...
    protected void linkEntity(PairKey pair, String conclusionId, String entityName) {
        var entity =
                entities.upsert(
                        WORKSPACE, entityName, null, embedder.embed(entityName, EmbedPurpose.ENTITY));
