...
노드의 정체는 표시 이름이 아니라 **정규화된 키**(`name_norm`)다. 앞뒤 공백을 벗기고, 소문자로 내리고,
가운데 공백을 하나로 줄인 값이다. 그래서 `"Seoul"`, `"seoul"`, `"  Seoul  "` 는 세 노드가 아니라
**한 노드**이고, 표기가 흔들려도 `ent` 신호가 흩어지지 않는다.

이 키는 쌍이 아니라 **워크스페이스 안에서** 유일하다 — `UNIQUE (workspace_name, name_norm)`. 노드는
공유된다. 서울을 언급하는 모든 쌍이 같은 행을 가리키고, 그래서 천 번 언급된 이름도 임베딩은 한 번이다.
쌍 스코프인 것은 **링크**(`entity_links`)이고 observer 와 observed 를 싣는 쪽도 거기다. 이 구분은
챙겨 둘 값어치가 있다. `countWeight` 는 엔티티가 걸린 결론 수로 할인하는데, 그 수를 질의하는 쌍 안이
아니라 워크스페이스 전체로 세었더니 `ent` 신호가 정확히 그것이 보상해야 할 엔티티에서 무너진 적이 있다.

같은 규칙이 빈 이름을 위험하게 만든다. `""` 와 `"   "` 와 `"\t"` 는 전부 같은 빈 키로 접히므로, 쓰레기가
여러 개 생기는 것이 아니라 **이름이 없는 노드 하나**가 생기고 빈 문자열을 달고 온 모든 결론이 거기
연결된다. 그런 노드는 링크가 있어서 orphan 청소가 지우지 않고, 벡터가 있어서 인덱스 자리를 차지하며,
서로 무관한 결론들에 `ent` 를 한꺼번에 나눠 주게 된다. 그래서 **빈 이름은 노드가 되기 전에 버려진다.**

버리는 것이지 거절하는 것이 아니다. 여기 오는 이름은 대부분 모델이 낸 것이고, 예외를 던져 봐야 쓰기가
되돌아가지도 않는다. 쓰기는 트랜잭션이 아니어서 결론은 이미 저장돼 있고, 엔티티 엣지만 빠진 채 남는다.
그 상태로 work unit 이 5회까지 재시도하며 같은 사실을 다시 도출하고, dedup 이 그것을 강화로 세어
`times_derived`(= `reinf` 신호)를 부풀린 뒤 배치가 격리된다. 사람이 HTTP 로 직접 주입할 때는 반대로
거절한다 — 클라이언트는 자기 버그를 고칠 수 있다 ([`guide.md` §4](guide.md#방법-b--결론을-직접-주입한다)).
