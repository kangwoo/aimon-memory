...
**오류 메시지는 당신이 보낸 것만 되돌려준다.** 4xx·422 는 당신의 요청을 인용한다 — 그게 쓸모의 전부다.
5xx 도 당신이 보낸 것은 되풀이할 수 있다 — `store_failed` 는 당신이 찾은 entity 나 session 이름을
말한다 — 그 이상은 없다. 저장된 값(파싱되지 않는 `metadata` 컬럼), 모델 제공자의 응답 본문
...
필요하면 운영자에게 그 시각의 로그를 물어라. 이건 정보를 아끼는 게 아니라, 응답 본문이 저장된 내용이
새어 나가는 통로가 되지 않게 하려는 것이다.
