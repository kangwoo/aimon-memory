...
| **추출 호출에서 엔티티를 공짜로** | mem0는 엔티티 추출을 별도로 한다. aimon-memory는 이미 도는 deriver 출력 스키마에 필드 하나를 더해 얻는다 (§4.2) |
...
aimon-memory는 엔티티 노드를 워크스페이스 단위로 두고 **엣지가 쌍 스코프를 들고 다닌다.**
...
Honcho의 최대 약점은 결론이 최대 30분 뒤에 보인다는 것이다. aimon-memory는 게이트를 하나 추가한다.
...
핵심 절약이다. mem0는 엔티티 추출을 별도 단계로 돌리지만, aimon-memory는 **이미 도는 추출 호출의 출력 스키마를 넓힌다.**
...
aimon-memory는 Tier 0·1을 순수 함수로 만들어 그 문제를 없앤다.
