...
```
녹화:  DYAD_LLM_MODE=record  → 요청·응답 쌍을 test-fixtures/llm/{hash}.json 에 저장
재생:  DYAD_LLM_MODE=replay  → 해시 매칭으로 재생, 미스는 테스트 실패
실행:  DYAD_LLM_MODE=live    → 실제 호출 (로컬·수동만)
```
...
│
├── dyad-core/         ← 의존 없음. 도메인 타입, SPI, PairKey, WorkUnitKey, NanoId
├── dyad-testkit/      ← core. 골든 픽스처 로더, LLM replay, 스텁 구현, Testcontainers 베이스
│
├── dyad-text/         ← core. Analyzer(Nori/Standard/Bigram), 정규화, jtokkit
├── dyad-embed/        ← core. 임베딩 클라이언트, 배치·절단·재시도
├── dyad-llm/          ← core. OpenAI/Anthropic 백엔드, 구조화 출력, 툴 루프, replay 훅
├── dyad-store/        ← core, text. jOOQ 리포지토리, Flyway, pgvector, 필터 DSL
│
├── dyad-recall/       ← store, text, embed. 융합 공식, BM25, 엔티티 부스트, explain
├── dyad-memory/       ← store, llm, embed, text. Deriver, 중복제거, Summarizer, Dreamer
│
├── dyad-worker/       ← memory, store. 큐 매니저, 컨슈머, Reconciler  [독립 실행]
└── dyad-api/          ← recall, memory, store. 컨트롤러, DTO, 인증, SSE  [독립 실행]
```

`dyad-worker`와 `dyad-api`는 **같은 코드베이스, 다른 실행 프로파일**이다. 배포는 두 개의 프로세스.
...
| 트랙 | 담당 모듈 | 성격 |
|---|---|---|
| **A · 영속성/API** | `dyad-store`, `dyad-api` | 스키마, 리포지토리, CRUD, 인증, 필터 DSL, OpenAPI |
| **B · 검색/텍스트** | `dyad-text`, `dyad-embed`, `dyad-recall` | Analyzer, BM25, 융합 공식, 엔티티 레이어, 평가셋 |
| **C · LLM/파이프라인** | `dyad-llm`, `dyad-memory`, `dyad-worker` | 프로바이더, 프롬프트, Deriver, 큐, Dialectic, Dreamer |

`dyad-core`와 `dyad-testkit`은 **공동 소유**다. 변경 시 3인 리뷰.
...
| B2.1 | `dyad-embed` — OpenAI 클라이언트, `encoding_format=float` 명시 |
...
| # | 작업 |
|---|---|
| C1.1 | `dyad-llm` — OpenAI 백엔드 (구조화 출력은 `create()` + `json_schema`) |
...
3. `dyad-core` SPI 6종을 첫 PR로 올려 3인 합의
