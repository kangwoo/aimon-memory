...
public class QueueRepository {
    ...
    public List<ReadyUnit> ready(String workspace, BatchSettings settings, int limit) {
        return jdbc.sql(
                ...
                .params(
                        ...
                        settings.maxAge().toSeconds(),
                        settings.idleFlush().toSeconds(),
                        limit)
                .query(
                        (rs, i) ->
                                new ReadyUnit(
                                        rs.getString("work_unit_key"),
                                        rs.getLong("tokens"),
                                        rs.getInt("items"),
    ...
    public boolean claim(String workUnitKey, String workerId, Duration ttl) {
        return jdbc.sql(
                        ...
                        """)
                .params(workUnitKey, workerId, ttl.toSeconds())
                .query(String.class)
                .optional()
                .isPresent();
    }
    ...
    public void release(String workUnitKey, String workerId) {
        jdbc.sql("DELETE FROM work_unit_claims WHERE work_unit_key = ? AND worker_id = ?")
                .params(workUnitKey, workerId)
                .update();
    }

    public void extendClaim(String workUnitKey, String workerId, Duration ttl) {
        jdbc.sql(
                        "UPDATE work_unit_claims SET expires_at = now() + (? * interval '1 second')"
                                + " WHERE work_unit_key = ? AND worker_id = ?")
                .params(ttl.toSeconds(), workUnitKey, workerId)
                .update();
    }
    ...
    public List<QueueItem> pending(String workUnitKey, int limit) {
        return jdbc.sql(
                        "SELECT * FROM queue WHERE work_unit_key = ? AND processed = FALSE"
                                + " ORDER BY id LIMIT ?")
                ...
                .query(
                        (rs, i) ->
                                new QueueItem(
                                        ...
                                        rs.getString("work_unit_key"),
                                        TaskType.fromWire(rs.getString("task_type")),
                                        Jsonb.toMap(rs.getString("payload")),
    ...
    public void recordFailure(List<Long> ids, String error) {
        ...
        jdbc.sql("UPDATE queue SET attempts = attempts + 1, last_error = ? WHERE id = ANY (?)")
                .params(error, ids.toArray(Long[]::new))
                .update();
    }
    ...
    public List<String> workspacesWithPendingWork(int limit) {
        return jdbc.sql(
                        "SELECT DISTINCT q.workspace_name FROM queue q WHERE q.processed = FALSE"
                                + " ORDER BY q.workspace_name LIMIT ?")
                .param(limit)
                .query(String.class)
                .list();
    }
    ...
    public int pendingCount(String workspace) {
        return jdbc.sql("SELECT count(*) FROM queue WHERE workspace_name = ? AND processed = FALSE")
                .param(workspace)
                .query(Integer.class)
                .single();
    }
    ...
    public Optional<Instant> oldestPending(String workspace) {
        return jdbc.sql(
                        "SELECT min(created_at) AS oldest FROM queue WHERE workspace_name = ? AND processed = FALSE")
                .param(workspace)
                .query((rs, i) -> rs.getTimestamp("oldest"))
                .optional()
