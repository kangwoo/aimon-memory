...
public class CollectionRepository {
    ...
    public Optional<DreamState> dreamState(PairKey pair) {
        return jdbc.sql(
                        "SELECT last_dream_at, explicit_at_last_dream FROM collections"
                                + " WHERE workspace_name = ? AND observer = ? AND observed = ?")
                .params(pair.workspaceName(), pair.observer(), pair.observed())
                .query(
                        (rs, i) ->
                                new DreamState(
                                        rs.getTimestamp("last_dream_at") == null
                                                ? null
                                                : rs.getTimestamp("last_dream_at").toInstant(),
    ...
    public void markDreamed(PairKey pair, int explicitCount) {
        jdbc.sql(
                        "UPDATE collections SET last_dream_at = now(), explicit_at_last_dream = ?"
                                + " WHERE workspace_name = ? AND observer = ? AND observed = ?")
                .params(explicitCount, pair.workspaceName(), pair.observer(), pair.observed())
                .update();
    }
}
