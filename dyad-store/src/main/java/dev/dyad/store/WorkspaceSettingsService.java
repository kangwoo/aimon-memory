...
public class WorkspaceSettingsService implements AnalyzerResolver {
    ...
    private static final Set<String> KNOWN_KEYS =
            Set.of(
                    ...
                    "recall.oversample",
                    "recall.entity_top_k",
                    "recall.entity_sim_cut",
                    "dedup.cosine_distance_max",
                    "dedup.unique_token_weight",
                    "batch.token_threshold",
                    "batch.max_age_minutes",
                    "batch.idle_flush_seconds",
                    "observe_me",
                    "observe_others");
        ...
        for (String key : config.keySet()) {
            if (KNOWN_KEYS.contains(key)) {
                throw new ConfigurationException(
                        "'" + key + "' is workspace configuration; set on a " + level
    ...
    public static void validate(Map<String, Object> config) {
        ...
        for (String key : config.keySet()) {
            if (!KNOWN_KEYS.contains(key)) {
                throw new ConfigurationException(
                        "unknown configuration key '" + key + "'; allowed: "
    ...
    private WorkspaceSettings load(String workspaceName) {
        ...
        try {
            ...
            log.warn(
                    "workspace {} has an invalid configuration and is falling back to defaults: {}",
                    workspaceName,
                    e.getMessage());
    ...
    private static WorkspaceSettings parse(Map<String, Object> config, boolean strict) {
        RecallSettings recall =
                new RecallSettings(
                        weights(config, strict),
                        bounded(config, "recall.half_life_days", RecallSettings.DEFAULT.halfLifeDays(),
                                0.0, false, 100_000.0, strict),
                        bounded(config, "recall.threshold", RecallSettings.DEFAULT.threshold(),
                                0.0, true, 1.0, strict),
                        (int) bounded(config, "recall.oversample", RecallSettings.DEFAULT.oversample(),
                                1.0, true, 100.0, strict),
                        (int) bounded(config, "recall.entity_top_k", RecallSettings.DEFAULT.entityTopK(),
                                1.0, true, 1_000.0, strict),
                        bounded(config, "recall.entity_sim_cut", RecallSettings.DEFAULT.entitySimCut(),
                                0.0, true, 1.0, strict));
        DedupSettings dedup =
                new DedupSettings(
                        bounded(config, "dedup.cosine_distance_max", DedupSettings.DEFAULT.cosineDistanceMax(),
                                0.0, true, 2.0, strict),
                        (int) bounded(config, "dedup.unique_token_weight", DedupSettings.DEFAULT.uniqueTokenWeight(),
                                0.0, true, 1_000.0, strict));
        ...
        BatchSettings batch =
                new BatchSettings(
                        (int) bounded(config, "batch.token_threshold", BatchSettings.DEFAULT.tokenThreshold(),
                                0.0, true, 1_000_000.0, strict),
                        Duration.ofMinutes(
                                (long) bounded(config, "batch.max_age_minutes", 30, 0.0, true, 10_080.0, strict)),
                        Duration.ofSeconds(
                                (long) bounded(config, "batch.idle_flush_seconds", 3, 0.0, true, 3_600.0, strict)));
        return new WorkspaceSettings(
                languageTag(config, strict),
                recall,
                dedup,
                batch,
                flag(config, "observe_me", true, strict),
                flag(config, "observe_others", true, strict));
    }
    ...
    private static FusionWeights weights(Map<String, Object> config, boolean strict) {
        ...
        if (!(raw instanceof List<?> list) || list.size() != 6) {
            if (strict) {
                throw new ConfigurationException(
                        "'recall.weights' expects six numbers in the order"
    ...
    private static double bounded(
        ...
        if (!aboveMin || d > max || Double.isNaN(d)) {
            if (strict) {
                throw new ConfigurationException(
                        "'" + key + "' must be in " + (minInclusive ? "[" : "(") + min + ", " + max
                                + "], got " + d);
            }
            return fallback;
        }
