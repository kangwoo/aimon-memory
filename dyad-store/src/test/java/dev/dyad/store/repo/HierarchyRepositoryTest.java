...
class HierarchyRepositoryTest extends StoreTestBase {
        ...
        try (ExecutorService pool = Executors.newVirtualThreadPerTaskExecutor()) {
            List<Callable<Long>> tasks =
                    IntStream.range(0, writers)
                            .<Callable<Long>>mapToObj(i -> () -> sessions.nextSequence(WORKSPACE, "s1", perWriter))
                            .toList();
    ...
    void messageBatchGetsContiguousSequence() {
        ...
        var saved =
                messages.insertBatch(
                        ...
                        start,
                        List.of(
                                new MessageRepository.NewMessage("alice", "one", 1, Map.of()),
    ...
    void messagesCanBeReadFromTheStartOrFromAnOffset() {
        ...
        assertThat(messages.inSession(WORKSPACE, "s1", null, 10))
                .extracting(m -> m.content())
                .containsExactly("one", "two", "three");
        assertThat(messages.inSession(WORKSPACE, "s1", 2L, 10))
                .extracting(m -> m.content())
    ...
    void replacingTheRosterClosesTheWindowsOfEveryoneOmitted() {
        ...
        sessionPeers.replace(
                WORKSPACE, "s1",
                List.of(
                        new SessionPeerRepository.Membership("alice", true, false),
                        SessionPeerRepository.Membership.of("dave")));

        Instant now = Instant.now().plusSeconds(1);
        assertThat(sessionPeers.membersAt(WORKSPACE, "s1", now))
                .extracting(p -> p.peerName())
