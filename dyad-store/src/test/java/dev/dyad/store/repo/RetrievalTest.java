...
class RetrievalTest extends StoreTestBase {
    ...
    void keywordCandidateSelectionSurvivesRowsMovingOnDisk() {
        ...
        List<String> before =
                conclusions.keyword(pair, Drafts.analyze("bank"), 5, Filter.ALL).stream()
                        .map(h -> h.id())
                        .toList();
        ...
        assertThat(conclusions.keyword(pair, Drafts.analyze("bank"), 5, Filter.ALL))
                .extracting(h -> h.id())
                .isEqualTo(before);
    }
    ...
    void byIdsExcludesSoftDeletedRows() {
        List<String> ids =
                conclusions.list(pair, Filter.ALL, 0, 10).items().stream().map(c -> c.id()).toList();
        ...
        assertThat(conclusions.byIds(WORKSPACE, ids))
                .hasSize(2)
                .extracting(c -> c.id())
                .doesNotContain(ids.get(0));
    ...
    void deletedAndExpiredRowsAreInvisible() {
        ...
        assertThat(conclusions.semantic(pair, Drafts.embed("bank"), 10, Filter.ALL))
                .extracting(h -> h.id())
                .doesNotContain(hit.id());

        jdbc.sql("UPDATE conclusions SET expires_at = now() - interval '1 day' WHERE deleted_at IS NULL")
                .update();
    ...
    void archiveCandidatesAreOnlyStaleUnreferencedSingletons() {
        ...
        conclusions.upsert(Drafts.explicit(pair, "s1", "alice works at a bank in seoul"));
        assertThat(conclusions.archiveCandidates(pair, cutoff, 10))
                .extracting(c -> c.content())
        ...
        String hiking =
                conclusions.list(pair, Filter.ALL, 0, 10).items().stream()
                        .filter(c -> c.content().contains("hiking"))
                        .findFirst()
                        .orElseThrow()
                        .id();
        jdbc.sql("UPDATE conclusions SET source_ids = ?::jsonb WHERE content LIKE 'bob%'")
                .param("[\"" + hiking + "\"]")
                .update();
        assertThat(conclusions.archiveCandidates(pair, cutoff, 10))
                .extracting(c -> c.content())
