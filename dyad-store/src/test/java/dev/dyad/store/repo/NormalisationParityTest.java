...
class NormalisationParityTest extends StoreTestBase {
    ...
    void storedColumnMatchesTheJavaRule() {
        ...
        String id = conclusions.upsert(dev.dyad.store.Drafts.explicit(pair, "s1", content)).conclusionId();

        List<String> mismatches =
                jdbc.sql(
                                "SELECT id FROM conclusions WHERE content_norm <> "
                                        + Normalizer.SQL_EXPRESSION.formatted("content"))
                        .query(String.class)
                        .list();
    ...
    private String normaliseInPostgres(String raw) {
        return jdbc.sql("SELECT " + Normalizer.SQL_EXPRESSION.formatted("?") + " AS n")
                .param(raw)
                .query(String.class)
                .single();
    }
