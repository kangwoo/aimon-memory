...
- **`docs/openapi.json` 이 빈 메시지 배치를 유효하다고 말하고 있었다 — 문서만 바뀌고 런타임은 그대로다.**
  `CreateMessages.messages` 에는 `@NotEmpty` 가 붙어 있어서 `{"messages":[]}` 는 처음부터 400 이었는데,
  발행 스키마는 `minItems: 0` 이었다. 범인은 springdoc 아래의 swagger-core 다.
  `ValidationAnnotationsUtils.applySizeConstraint` 가 `setMinItems(min)` 을 아무 조건 없이 부르고, 그
  호출이 `@NotEmpty` 가 세워 둔 `minItems: 1` **뒤에** 온다. 그래서 `@Size(max = 100)` 만 적힌 필드는
  천장을 얻는 값으로 바닥을 잃는다. `@Size(min = 1, max = 100)` 으로 바꿔 `AddSessionPeers.peers` 의
  선례를 따랐다. 더 깨끗해 보이는 길은 없다 — `@Schema(minItems = 1)` 도 같은 덮어쓰기에 지고,
  `@NotEmpty` 를 떼면 `minItems: 1` 은 실리지만 `required` 가 함께 사라지고 null 배열이 통과한다.
...
- **주입 결론의 길이에 상한이 생겼다 — `Requests.MAX_CONCLUSION_CHARS`, 800자, 넘으면 400.**
  *(같은 `Unreleased` 안에서 대체됨 — 인덱스가 `md5(content_norm)` 위로 옮겨가 800 을 강요하던
  ...
  2704바이트를 넘을 수 없다. 메시지에는 그런 인덱스가 없다 — `ix_message_fts` 와 `ix_message_trgm` 은
  둘 다 GIN 이라 값 전체가 아니라 텀을 색인한다. 실제 스키마에 API 로 넣어, 압축되지 않는 텍스트로
  측정한 값: 결론 한글 890자 200 / 900자 **500**, 라틴 2650자 200 / 2680자 **500**,
  메시지 한글 32000자 200.
  그 500 은 `index row size 2728 exceeds btree version 4 maximum 2704`, SQLSTATE 54000 이고
  `DataIntegrityViolationException` 이 아니라(class 54 는 `DataAccessResourceFailureException` 으로
  번역된다) `ApiExceptionHandler` 의 catch-all 까지 내려가 `internal_error` 로 답하고 ERROR 로
  로깅됐다 — 클라이언트의 긴 필드가 서버 장애로 집계됐다.
  **경계는 하나의 숫자가 아니라 구간이다.** `index_form_tuple` 이 엔트리를 재기 전에 속성을 압축하므로
  몇 자가 들어가는지는 그 텍스트가 얼마나 압축되는지에 달렸다 — 같은 음절만 32000자 반복한 결론은
  아무 문제 없이 저장된다. 위아래의 모든 숫자는 반복이 없는 텍스트로 잰 것이고, 상한을 세울 수 있는
  쪽은 그쪽뿐이다.
  800 의 근거는 2704 를 한글·한자 한 글자의 UTF-8 3바이트로 나눈 901 이 **아니다.** 같은 엔트리에 이름
  세 칸이 함께 들어가고 이름에는 상한이 없기 때문이다. 공백 없이 3바이트 문자로만 채운 최악의 경우를
  이름 길이별로 측정하면 800자는 이름 255바이트까지 들어가고 825자는 들어가지 않는다.
  **이전에 통과하던 요청을 거절하게 된다** — 라틴 문자 1000자짜리 결론은 지금까지 200 이었고 이제
  400 이다. 한 필드가 두 개의 상한을 가질 수는 없고, 2600자를 허용하는 상한은 이 저장소의 코퍼스와
  프롬프트와 명세가 실제로 쓰인 언어에서는 아무것도 막지 못한다.
  **한 글자 3바이트가 최악이다.** `@Size` 는 UTF-16 코드 단위를 세고 보조 평면 문자는 4바이트짜리
  두 단위이므로, 세는 단위당 2바이트로 한글 한 글자보다 **싸다.** 측정: 보조 평면 문자로 채운 800자는
  1600바이트이고 저장된다. "이모지 800개"는 세면 1600자라 검증기가 먼저 400 을 답한다. 그래서 이 상한은
  content 를 2400바이트로 묶고, 이 필드로 들어오는 어떤 입력도 그 위로 못 간다.
  **행이 저장된다는 보장은 아니다.** 엔트리는 이 필드만이 아니라 content 의 바이트 + 이름 세 칸 +
  튜플 오버헤드 15바이트이고, 상한에서 이름 세 칸에 남는 몫은 대략 280바이트다 — 측정하면 이름
  280바이트는 저장되고 285바이트는 안 된다. 그걸 강제하는 것은 아무것도 없다. `observer` 와
  `observed` 는 같은 요청 본문의 상한 없는 필드라서, 이름이 1200바이트인 쌍에 100자짜리 결론을 넣으면
  여전히 500 이다. 이 상한은 `content` 를 그 실패에서 빼내는 것이지 경로를 닫는 것이 아니다.
  닫으려면 인덱스를 바꾸거나 `ConclusionWriter` 를 막아야 하는데, 후자는 deriver 의 출력까지
  덮는 유일한 자리이면서 이 필드의 제약으로는 닿을 수 없다. 열어 둔 채 적어 뒀다.
  `docs/openapi.json` 에 `maxLength: 800` 한 줄로 실린다.
...
- **위 두 항목이 닫지 않는 것.** 결론의 **길이**가 저장을 깨지 못하게 된 것은 **이 API 가 받는 길이
  ...
  **이 항목의 규칙은 아래 항목이 대체했다.** 릴리스 시점 기준으로 `:` 는 더 이상 제거되지 않고
  `note:draft` 는 자기 문서를 찾으며, 아포스트로피는 위치와 무관하게 남는다. 위 문단은 이 저장소가
  허용 목록에서 피연산자 인용으로 옮겨 간 중간 단계이고, 그 단계에서 무엇이 왜 남았는지의 기록이다.
- **키워드 질의어를 이제 깎지 않고 인용부호로 감싼다. 구두점·결합 문자·보충 평면 문자가 붙은 토큰이
  자기 문서를 못 찾던 문제.** `TsQuery` 는 tsquery 연산자가 될 수 있는 문자를 전부 지워서 보냈는데,
  색인과 질의는 같은 분석기의 출력이라 지우는 순간 자기가 분석돼 나온 문서와 어긋난다. `note:draft` 는
  `notedraft` 로 나가 `'note':1 'draft':2` 와 매칭되지 않았다. 이제 항마다 `'…'` 로 감싸고 인용부호와
  역슬래시만 두 배로 이스케이프한다 — 인용 어휘소 안에서 뜻을 가지는 문자가 그 둘뿐이기 때문이다.
  Postgres 16.15 에서 실측: 32..126 의 영숫자 아닌 33개 문자를 `a<c>b` 로 넣으면 33개 전부 파싱되고
  33개 전부 자기 문서와 매칭된다(예전 규칙은 5개). 제어문자 32개도 마찬가지다.

  **아무도 세어 보지 않았던 쪽이 더 크다.** 예전 코드는 `char` 단위로 `Character.isLetterOrDigit` 를
  봤는데, 결합 문자(데바나가리 비라마, 타이 모음, 히브리 니쿠드, 아랍 하라카트, NFD 악센트)와 보충 평면
  코드 포인트가 둘 다 그 술어 바깥이다. `नमस्ते` 는 `नमसत` 로 나갔고 `𠀀` 는 아예 빈 문자열이 되어 질의에서
  통째로 빠졌다. 분석기 양쪽 모두 이 토큰들을 그대로 내보내므로, 해당 질의는 **아무것도** 매칭하지 못했다.
  이제 매칭된다. **출력 가능 ASCII 안에서** 반대 방향은 딱 하나, **앞자리 `~`** 다. 예전 규칙이 찾던
  행을 인용형이 못 찾는다. `1..127` 을 네 위치(단독·앞·뒤·워드 내부)에 넣고 예전 매칭과 새 매칭을
  비교하면 단독·뒤·워드 내부는 후퇴 0건(워드 내부는 60건 개선)이고 앞자리만 정확히 1건, `~` 다. 이
  빌드의 분석기 셋은 `~b` 를 모두 `[b]` 로 내놓으므로 도달 불가능하고, 대가는 문장이 아니라 행 하나다.
  ASCII 밖에서는 같은 방향의 손실이 하나 더 있다. 항의 표식(mark)을 떼어낸 형태가 어떤 문서에 우연히
  들어 있던 경우다 — NFD `café` 는 예전에 `cafe` 로 나가 `cafe latte` 를 담은 행과 **매칭됐고**,
  인용형 `'café'` 는 매칭되지 않는다. 같은 실측이 무엇과 맞바꿨는지도 보여 준다: 예전 형태는 정작 자기가
  분석돼 나온 행을 못 찾았고 인용형은 찾는다. 질의가 쓰지도 않은 철자에 우연히 걸리던 매칭이라, 잃는
  것이 곧 수정이다.

  **곡선 아포스트로피(`U+2019`)도 새로 살아남고, 이쪽은 실제 텍스트에서 흔하다.** 표준 토크나이저는
  ASCII 아포스트로피와 똑같이 이 문자를 단어 안에 남기므로 `don’t` 는 토큰 하나다. 예전 규칙은
  `dont` 를 보내 아무것도 못 찾았고, 피연산자 `'don’t'` 는 `'don' <-> 't'` 로 파싱돼 매칭된다.

- **`corpusStats` 의 df 가 키워드 경로가 세는 것보다 큰 집합을 세던 문제.** df 는 `plainto_tsquery` 로
  셌는데 그건 토큰의 어휘소를 `&` 로 잇고, 후보 질의는 `<->` 로 잇는다. 두 어휘소를 인접하지 않은 자리에
  가진 행이 후보가 될 수 없으면서 df 에는 잡혔다 — 3행 코퍼스에서 df 3 대 매칭 2행. 이제 df 도
  `TsQuery` 가 만드는 것과 같은 피연산자로 센다. 어휘소가 둘 이상으로 쪼개지는 토큰만 해당한다:
  `alice's`·`50,000`·`snake_case`·`foo-bar`·`note:draft` 는 달라지고, `e.g`·`192.168.0.1`·`a@b.com`
  같은 단일 어휘소는 예전에도 같은 값이었다.

  **다중 어휘소 토큰이 든 질의의 BM25 점수가 전부 움직인다.** df 가 내려가니 IDF 가 올라간다 —
  3행 코퍼스에서 `idf(3,3)=0.133531` 이 `idf(3,2)=0.470004` 로, N=1000 에 df 300→200 이면
  `1.203307` 이 `1.607941` 로. 한 항짜리 질의의 순서는 뒤집히지 않지만(그 항의 기여가 모든 문서에서
  같은 배수로 커진다), 여러 항이 섞이면 상대 가중치가 바뀌고 `KeywordSignal.normalise` 는 로지스틱이라
  스케일 불변이 아니다. 골든 픽스처와 랭킹 기준선은 움직이지 않았다 — 이유는 실측했다: eval 코퍼스와
  질의가 만드는 Nori 토큰 180개 전부가 영숫자로만 이루어져 있어서 두 변경 다 그 코퍼스에서는 무연산이다.

  **거절되던 요청이 통과하게 되는 변경은 없고, 오류 코드도 그대로다.** `TsQuery.orOf` 는 발행 모듈의
  공개 API 이고 출력 문자열이 바뀐다 — `orOf(["alice"])` 는 이제 `'alice'` 다.
