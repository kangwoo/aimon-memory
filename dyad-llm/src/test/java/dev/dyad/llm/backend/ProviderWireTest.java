...
class ProviderWireTest {
    ...
    private ChatCall structuredCall() {
        return new ChatCall(
                null,
                "You extract facts.",
                List.of(new ChatTurn.UserText("where does alice work")),
                List.of(),
                ResponseFormat.strict("answer", SCHEMA),
                0.0,
                null);
    }
    ...
    void anthropicDeclaresTheSchemaThroughOutputConfig() {
        AnthropicChatBackend backend =
                new AnthropicChatBackend(baseUrl(), "k", "claude-opus-5", Duration.ofSeconds(5));
    ...
    void anthropicKeepsToolsAlongsideAStructuredFormat() {
        ...
        backend.chat(
                new ChatCall(
                        null,
                        "system",
                        List.of(new ChatTurn.UserText("hi")),
                        List.of(new ToolSpec("recall", "look things up", SCHEMA)),
                        ResponseFormat.strict("answer", SCHEMA),
                        0.0,
                        null));
    ...
    void anthropicOmitsTemperatureOnModelsThatRejectIt() {
        for (String model : List.of("claude-opus-5", "claude-opus-4-8", "claude-sonnet-5", "claude-fable-5")) {
            ...
            assertThat(captured.get().has("temperature"))
                    .as("%s rejects sampling parameters with a 400", model)
                    .isFalse();
        }
    }
    ...
    void anthropicKeepsTemperatureOnModelsThatStillTakeIt() {
        new AnthropicChatBackend(baseUrl(), "k", "claude-sonnet-4-6", Duration.ofSeconds(5))
                .chat(structuredCall());
    ...
    void anthropicGivesStructuredCallsRoomToFinish() {
        ...
        backend.chat(
                new ChatCall(
                        null, "system", List.of(new ChatTurn.UserText("hi")), List.of(), null, 0.0, 256));
    ...
    void openAiDeclaresAStrictJsonSchema() {
        OpenAiChatBackend backend =
                new OpenAiChatBackend(baseUrl(), "k", "gpt-4.1-mini", Duration.ofSeconds(5));
