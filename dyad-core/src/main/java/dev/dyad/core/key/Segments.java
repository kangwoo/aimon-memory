...
final class Segments {
    ...
    static String decode(String encoded) {
        ...
        for (int i = 0; i < encoded.length(); i++) {
            ...
            if (i + 2 >= encoded.length()) {
                throw new DyadException("bad_key", "truncated escape in key segment: " + encoded);
            }
                    ...
                    switch (hex) {
                        ...
                        default ->
                                throw new DyadException(
                                        "bad_key", "unsupported escape %" + hex + " in key: " + encoded);
                    });
            i += 2;
        }
        return sb.toString();
    ...
    static String required(String value, String field) {
        if (value == null || value.isBlank()) {
            throw new DyadException("bad_key", field + " must not be blank");
        }
        return value;
    }
}
