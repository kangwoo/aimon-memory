 ...
 */
public record WorkUnitKey(
        TaskType taskType, String workspaceName, String sessionName, String observer, String observed) {

    public WorkUnitKey {
        if (taskType == null) {
            throw new DyadException("bad_key", "taskType must not be null");
        }
    ...
    public static WorkUnitKey representation(String workspace, String session, PairKey pair) {
        return new WorkUnitKey(
                TaskType.REPRESENTATION, workspace, session, pair.observer(), pair.observed());
    }
    ...
    public String encode() {
        return taskType.wire()
                + ':'
                + Segments.encode(workspaceName)
                + ':'
                + Segments.encode(sessionName)
                + ':'
                + Segments.encode(observer)
                + ':'
                + Segments.encode(observed);
    }
    ...
    public static WorkUnitKey parse(String encoded) {
        ...
        if (parts.length != 5) {
            throw new DyadException(
                    "bad_key", "work unit key needs 5 segments, got " + parts.length + ": " + encoded);
        }
        String session = Segments.decode(parts[2]);
        return new WorkUnitKey(
                TaskType.fromWire(parts[0]),
                Segments.decode(parts[1]),
                session.isEmpty() ? null : session,
                Segments.decode(parts[3]),
                Segments.decode(parts[4]));
    }
