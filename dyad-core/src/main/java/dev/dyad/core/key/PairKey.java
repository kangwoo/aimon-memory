...
public record PairKey(String workspaceName, String observer, String observed) {
    ...
    public String encode() {
        return Segments.encode(workspaceName)
                + ':'
    ...
    public static PairKey parse(String encoded) {
        ...
        if (parts.length != 3) {
            throw new DyadException("bad_key", "pair key needs 3 segments, got " + parts.length + ": " + encoded);
        }
        return new PairKey(
                Segments.decode(parts[0]), Segments.decode(parts[1]), Segments.decode(parts[2]));
    }
