...
public record ConclusionDraft(
    ...
    public ConclusionDraft {
        if (pair == null) {
            throw new DyadException("bad_draft", "pair is required");
        }
        if (content == null || content.isBlank()) {
            throw new DyadException("bad_draft", "content is required");
        }
        if (level == ConclusionLevel.EXPLICIT && (sessionName == null || sessionName.isBlank())) {
            throw new DyadException("bad_draft", "explicit conclusions require a session");
        }
