...
class ApiRoundTripTest extends ApiTestBase {
    ...
    void messagesAreStoredAndReturnedInSequence() throws Exception {
        String response =
                postJson(
                        "/v1/workspaces/ws/sessions/s1/messages",
                        """
                        {"messages":[
    ...
    void entityProvenanceReachesTheOriginalMessages() throws Exception {
        ...
        mvc.perform(get("/v1/workspaces/ws/recall/provenance")
                        .param("entity", "seoul")
                        .param("observer", "alice")
                        .param("observed", "alice")
                        .header("Authorization", bearer(token)))
                .andExpect(status().isOk())
    ...
    void deletingAConclusionIsRecordedAndHidesItFromRecall() throws Exception {
        ...
        String recalled =
                postJson(
                        "/v1/workspaces/ws/recall", "{\"query\":\"bank\",\"observer\":\"alice\",\"observed\":\"alice\"}");
    ...
    void aConstraintViolationIsAConflictNotAServerError() throws Exception {
        mvc.perform(
                        org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .post("/v1/workspaces/ws/sessions/s1/peers")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"peers\":[{\"peer\":\"ghost\"}]}")
                                .header("Authorization", bearer(token)))
                .andExpect(status().isOk());
        ...
        mvc.perform(
                        org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .post("/v1/workspaces/ws/sessions/never-created/peers")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"peers\":[{\"peer\":\"ghost\"}]}")
                                .header("Authorization", bearer(token)))
                ...
                .andExpect(jsonPath("$.message").value(
                        org.hamcrest.Matchers.not(org.hamcrest.Matchers.containsString("session_peers"))));
    }
    ...
    void aMalformedBodyIsABadRequest() throws Exception {
        mvc.perform(
                        post("/v1/workspaces/ws/recall")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"observer\":\"alice\",\"observed\":\"alice\"}")
                                .header("Authorization", bearer(token)))
    ...
    void workspaceConfigurationRoundTripsAndTakesEffect() throws Exception {
        mvc.perform(
                        org.springframework.test.web.servlet.request.MockMvcRequestBuilders.put("/v1/workspaces/ws/configuration")
                                .contentType(MediaType.APPLICATION_JSON)
