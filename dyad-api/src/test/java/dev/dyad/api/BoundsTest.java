...
class BoundsTest extends ApiTestBase {
    ...
    void anAbsurdPageSizeIsClampedRatherThanHonoured() throws Exception {
        mvc.perform(get("/v1/workspaces").param("size", "10000000").header("Authorization", bearer(token)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.size").value(Bounds.MAX_PAGE_SIZE));
    }
    ...
    void recallLimitIsCapped() throws Exception {
        for (int i = 0; i < 5; i++) {
            mvc.perform(post("/v1/workspaces/ws/conclusions")
        ...
        mvc.perform(post("/v1/workspaces/ws/recall")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", bearer(token))
                        .content("{\"query\":\"alice\",\"observer\":\"a\",\"observed\":\"a\","
                                + "\"limit\":100000000}"))
                .andExpect(status().isOk());
    ...
    void theRecallRequestClampsItsOwnLimit() {
        var pair = new dev.dyad.core.key.PairKey("ws", "a", "a");
        assertThat(new dev.dyad.recall.RecallRequest(pair, "q", 100_000, null, null, false).limit())
                .isEqualTo(dev.dyad.recall.RecallRequest.MAX_LIMIT);
        assertThat(new dev.dyad.recall.RecallRequest(pair, "q", 0, null, null, false).limit())
                .isEqualTo(dev.dyad.recall.RecallRequest.DEFAULT_LIMIT);
    }
