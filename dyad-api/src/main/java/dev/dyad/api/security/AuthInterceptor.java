...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public static final String PRINCIPAL_ATTRIBUTE = "dyad.principal";
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        DyadPrincipal principal = jwt.verify(bearerToken(request));
        ...
        if (rule == null) {
            throw new ForbiddenException(
                    "route " + request.getMethod() + " " + pattern
        ...
        if (!principal.scope().satisfies(rule.minimumScope())) {
            ...
            boolean memberReadAllowed =
                    rule.memberRead()
                            && principal.scope() == TokenScope.SESSION
                            && principal.allowMemberRead();
            if (!memberReadAllowed) {
                throw new ForbiddenException(
                        "this token is " + principal.scope().wire() + "-scoped; the route requires "
                                + rule.minimumScope().wire());
            }
        }
    ...
    private void checkPathScope(HttpServletRequest request, DyadPrincipal principal) {
