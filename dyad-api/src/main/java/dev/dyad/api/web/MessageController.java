...
public class MessageController {
    ...
    public MessageController(
            MessageIngestionService ingestion, MessageRepository messages, ContextService context) {
    ...
    public List<Dtos.MessageResponse> create(
        ...
        List<MessageIngestionService.IncomingMessage> incoming =
                body.messages().stream()
                        .map(m -> new MessageIngestionService.IncomingMessage(m.peer(), m.content(), m.metadata()))
                        .toList();
    ...
    public Dtos.PageResponse<Dtos.MessageResponse> list(
            @PathVariable String workspace,
            @PathVariable String session,
    ...
    public Dtos.PageResponse<Dtos.MessageResponse> search(
            @PathVariable String workspace,
            @PathVariable String session,
            @RequestBody(required = false) Requests.SearchMessages body) {
        Requests.SearchMessages request =
                body == null ? new Requests.SearchMessages(null, 0, 50) : body;
        return Dtos.PageResponse.of(
                messages.search(
                        workspace,
                        session,
                        ...
                        Bounds.size(request.size() == null ? 50 : request.size())),
                Dtos.MessageResponse::of);
    }
    ...
    public Dtos.ContextResponse context(
            @PathVariable String workspace,
            @PathVariable String session,
            @RequestParam(defaultValue = "4000") int tokens,
            @RequestParam(required = false) String target,
            @RequestParam(required = false) String perspective) {

        var result = context.context(workspace, session, tokens);
        return new Dtos.ContextResponse(
                result.summary(),
                result.messages().stream().map(Dtos.MessageResponse::of).toList(),
                result.messagesStartSeq(),
