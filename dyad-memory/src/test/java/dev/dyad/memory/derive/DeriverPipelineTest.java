...
class DeriverPipelineTest extends MemoryTestBase {
    ...
    private List<Message> seedMessages(String session, String peer, String... contents) {
        ...
        List<MessageRepository.NewMessage> rows =
                java.util.Arrays.stream(contents)
                        .map(c -> new MessageRepository.NewMessage(peer, c, 5, Map.of()))
                        .toList();
    ...
    void extractionBecomesConclusionsEntitiesAndEvents() {
        ...
        var stored = conclusions.list(pair, dev.dyad.core.filter.Filter.ALL, 0, 10).items();
        ...
        String bankId =
                stored.stream().filter(c -> c.content().contains("bank")).findFirst().orElseThrow().id();
        assertThat(entities.entitiesFor(WORKSPACE, bankId))
                .extracting(e -> e.nameNorm())
                .containsExactlyInAnyOrder("seoul", "alice");

        assertThat(events.history(WORKSPACE, bankId, 10))
                .singleElement()
    ...
    void oneExtractionFansOutToEveryObservingPair() {
        ...
        assertThat(conclusions.list(self, dev.dyad.core.filter.Filter.ALL, 0, 10).total()).isEqualTo(2);
        assertThat(conclusions.list(observer, dev.dyad.core.filter.Filter.ALL, 0, 10).total()).isEqualTo(2);
    }
    ...
    void thePromptCarriesThePairPerspective() {
        ...
        var stub = new dev.dyad.testkit.stub.StubLlmClient(request -> TWO_CONCLUSIONS);
    ...
    void theAuditTrailNamesThePromptThatProducedTheFact() {
        ...
        assertThat(events.history(WORKSPACE, stored.get(0).id(), 10))
                ...
                .satisfies(e -> assertThat(e.detail())
                        .containsEntry("prompt_version", dev.dyad.memory.prompt.Prompts.VERSION)
    ...
    void replacingAConclusionKeepsTheEntityNodeItReattaches() {
        ...
        deriverReturning(
                        "{\"conclusions\":[{\"content\":\"alice works at a bank in seoul gangnam district"
        ...
        var live = conclusions.list(pair, dev.dyad.core.filter.Filter.ALL, 0, 10).items();
    ...
    void anEmptyExtractionWritesNothing() {
        ...
        assertThat(conclusions.list(pair, dev.dyad.core.filter.Filter.ALL, 0, 10).total()).isZero();
    }
    ...
    void blankConclusionsAreDiscarded() {
        ...
        var result =
                deriverReturning("{\"conclusions\":[{\"content\":\"   \",\"entities\":[]}]}")
                        .deriveAndWrite(pair, "s1", batch);
        assertThat(result.outcomes()).isEmpty();
    }
