...
class DialecticReplayTest extends MemoryTestBase {
    ...
    private DialecticService dialectic(ScriptedBackend backend, LlmFixtureStore fixtures, LlmMode mode) {
        return new DialecticService(
                new DefaultLlmClient(new RecordingChatBackend(backend, fixtures, mode)), tools);
    }

    private DialecticService.Question question() {
        return new DialecticService.Question(
                pair, "s1", "where does alice work?", List.of(), ReasoningLevel.LOW, null);
    }
    ...
    void theRecallToolReturnsRankedConclusionsWithTheirReinforcementCounts(@TempDir Path directory) {
        ...
        ToolLoopResult result =
                dialectic(backend, new LlmFixtureStore(directory), LlmMode.RECORD).answer(question());
    ...
    void anUnknownToolNameIsReportedToTheModel(@TempDir Path directory) {
        ScriptedBackend backend =
                new ScriptedBackend()
                        .requestingTool("call-1", "nonexistent_tool", "{}")
    ...
    void theReasoningLevelBoundsTheToolsOffered(@TempDir Path directory) {
        ...
        dialectic(backend, new LlmFixtureStore(directory), LlmMode.RECORD)
                .answer(
                        new DialecticService.Question(
                                pair, "s1", "q", List.of(), ReasoningLevel.MINIMAL, null));

        assertThat(backend.calls().get(0).tools())
                .extracting(t -> t.name())
                .containsExactly(ToolRegistry.RECALL);
    }
    ...
    void toolsCannotReachAnotherPairsMemory(@TempDir Path directory) {
        ...
        ToolLoopResult result =
                new DialecticService(
                                new DefaultLlmClient(
                                        new RecordingChatBackend(
                                                backend, new LlmFixtureStore(directory), LlmMode.RECORD)),
                                tools)
                        .answer(
                                new DialecticService.Question(
                                        other, "s1", "where does alice work?", List.of(), ReasoningLevel.LOW, null));
