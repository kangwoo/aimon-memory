...
class FindingsRenderingTest {
        ...
        for (int i = 0; i < count; i++) {
            calls.add(
                    new ToolCall(
                            "search_conclusions",
                            "{\"query\":\"q" + i + "\"}",
                            "finding " + i + " " + "가나다라마바사 ".repeat(charsEach),
                            false));
        }
        return calls;
    }
    ...
    void theFramingSurvivesATruncatedBody() {
        ...
        assertThat(TokenCounter.count(rendered))
                .as("the whole message still fits the budget it was given")
                .isLessThanOrEqualTo(DialecticService.MAX_FINDINGS_TOKENS);
        assertThat(rendered).startsWith("What the tools returned so far:");
        assertThat(rendered)
                .as("the model has to be told the search was cut short, especially when it was")
                .contains("cut short at its iteration limit")
                .contains("Answer from these results alone");
    }
    ...
    void failedCallsAreLabelled() {
        String rendered =
                DialecticService.renderFindings(
                        List.of(new ToolCall("grep_messages", "{}", "timed out", true)));
