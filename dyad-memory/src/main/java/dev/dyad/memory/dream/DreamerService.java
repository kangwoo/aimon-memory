...
public class DreamerService {
    ...
    public void run(DreamRepository.Dream dream) {
        ...
        try {
            ...
            int produced = 0;
            produced += specialist(pair, dream.id(), facts, validIds, Prompts.DREAM_DEDUCTION, ConclusionLevel.DEDUCTIVE);
            produced += specialist(pair, dream.id(), facts, validIds, Prompts.DREAM_INDUCTION, ConclusionLevel.INDUCTIVE);
            produced +=
                    specialist(
                            pair, dream.id(), facts, validIds, Prompts.DREAM_CONTRADICTION, ConclusionLevel.CONTRADICTION);
    ...
    private int specialist(
        ...
        DreamOutput output =
                llm.structured(
                                new LlmRequest(
                                        null, prompt, List.of(LlmMessage.user(facts)), 0.0, null,
                                        ResponseFormat.strict("dream_" + level.wire(), SCHEMA)),
                                DreamOutput.class)
                        .value();
        ...
        for (DreamOutput.Item item : output.conclusions()) {
            ...
            items.add(
                    new ConclusionWriter.Incoming(
                            ...
                            item.entities(),
                            level,
                            level == ConclusionLevel.INDUCTIVE ? confidence(item) : null,
                            sources,
                            List.of(),
