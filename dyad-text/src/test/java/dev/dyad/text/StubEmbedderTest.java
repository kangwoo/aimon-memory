...
class StubEmbedderTest {
    ...
    void koreanTextsSharingAPlaceNameAreRelated() {
        ...
        assertThat(cosine(
                        whitespace.embed("강남의 은행", EmbedPurpose.DOCUMENT),
                        whitespace.embed("강남 은행", EmbedPurpose.QUERY)))
                .as("the old behaviour, kept available for tests that want it")
                .isLessThan(0.6);
    }
    ...
    void unrelatedKoreanTextsStayUnrelated() {
        StubEmbedder embedder = new StubEmbedder();
        assertThat(cosine(
                        embedder.embed("강남 은행", EmbedPurpose.DOCUMENT),
                        embedder.embed("제주 등산", EmbedPurpose.QUERY)))
                .isLessThan(0.2);
    }
